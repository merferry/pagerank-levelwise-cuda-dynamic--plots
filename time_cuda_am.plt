...
plot 'time-am.csv' \
      using ($1-0.20):10 title 'DT_{MLPR}'       with boxes fill pattern 1, \
  ''  using 1:16:xtic(2) title 'DT_{LWPR}'       with boxes fill pattern 2, \
  ''  using ($1+0.20):18 title 'DT_{HYPR (GPU)}' with boxes fill pattern 3 noborder, \
  ''  using ($1+0.40):4  title 'ND_{NVPR}'  with boxes fill pattern 3 noborder, \
